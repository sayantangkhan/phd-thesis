...
As a corollary of the Theorem \ref{thm:mixing-bms}, we get the following results about the Patterson-Sullivan measures and the action of $\Gamma$ on the limit set $\Delta$.
\begin{corollary}
  \label{cor:niceness}
  ...
  \begin{itemize}
  \item There exists a unique family of conformal densities $\left\{ \mu_x \right\}_{x \in \teich(\os_g)}$ on $\Delta$, given by the Patterson-Sullivan construction.
  \item The action of $\Gamma$ on the $\Delta$ and $\Delta \times \Delta$ is ergodic.
  \end{itemize}
\end{corollary}
